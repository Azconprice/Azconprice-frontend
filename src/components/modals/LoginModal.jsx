...
import { saveTokensAndUserDetails } from "@/utils/auth";
import BaseModal, {
  modalInputStyles,
  modalButtonStyles,
  modalErrorStyles,
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        saveTokensAndUserDetails(accessToken, refreshToken);

        setEmail("");
        setPassword("");
        setRememberMe(false);

        onSuccess("Uğurla hesaba daxil oldunuz");
          ...
          case 400:
            if (errorData.errors) {
              if (errorData.errors.email) {
                setEmailError(
                  errorData.errors.email[0] || "Elektron poçt xətası."
                );
              }
              if (errorData.errors.password) {
                setPasswordError(
                  errorData.errors.password[0] || "Şifrə xətası."
                );
              }
