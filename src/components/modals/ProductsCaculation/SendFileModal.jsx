...
const SendFileModal = ({
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/Excel/upload?IsSimple=true`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        try {
          ...
          const columnMap = {
            queryName: headers.findIndex(
              (h) => h && (h.includes("Sorğu") || h.includes("Adı"))
            ),
            quantity: headers.findIndex((h) => h && h.includes("Miqdar")),
            matchingProducts: headers.findIndex(
              (h) => h && (h.includes("Uyğun") || h.includes("Məhsul"))
            ),
            averagePrice: headers.findIndex(
              (h) => h && (h.includes("Orta") || h.includes("Qiymət"))
            ),
          ...
          const processedRows = jsonData
            ...
            .map((row, index) => {
              ...
              return {
                id: index + 1,
                queryName:
                  (columnMap.queryName >= 0
                    ? row[columnMap.queryName]
                    : row[0]) || "",
                quantity:
                  (columnMap.quantity >= 0
                    ...
                    : row[1]) || "",
                matchingProducts:
                  (columnMap.matchingProducts >= 0
                    ? row[columnMap.matchingProducts]
                    : row[2]) || "",
                averagePrice:
                  (columnMap.averagePrice >= 0
                    ? row[columnMap.averagePrice]
                    : row[3]) || "",
                medianPrice:
                  (columnMap.medianPrice >= 0
                    ? row[columnMap.medianPrice]
                    : row[4]) || "",
                total:
                  (columnMap.total >= 0 ? row[columnMap.total] : row[5]) || "",
              };
            })
            .filter((row) => row !== null);
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-white text-[32px] font-semibold mb-6 text-center">
          Faylı göndərin
        </h2>
        <div
          {...getRootProps()}
        ...
        >
          <input {...getInputProps()} />
          {isDragActive ? (
            <p className="text-[#F37321] text-[24px] font-semibold">
              Faylı buraya buraxın...
            </p>
          ) : (
            <div className="my-[30px] flex flex-col items-center">
              ...
              />
              <p className="text-[#868686] text-[24px] font-semibold">
                Faylınızı sürükləyin
                <br />
                və ya
                <br />
                <span className="text-[#F37321] font-semibold">
        ...
        <p className="text-[#fff] text-[16px] mb-6 text-left">
        ...
        <div className="flex items-center bg-[#fff] px-3 justify-between py-3 rounded-xl mb-6 overflow-hidden ">
          <div className="flex items-center">
            ...
            />
            <span className="text-[14px] text-[#686868] w-full">
        ...
        <div className="mb-6 max-h-48 overflow-y-auto">
          {uploadedFiles.map((file, index) => (
            <div
              key={index}
              className="flex items-start md:items-center flex-col md:flex-row justify-between py-3 mb-2"
            >
              <div className="flex items-center">
                {file.status === "done" && (
                  <img
                ...
                <span className="text-gray-400 text-sm pl-[20px]">
